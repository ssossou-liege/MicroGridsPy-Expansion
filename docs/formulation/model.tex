...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{system_config.png}
    \caption{microgrid system architecture}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{scenario_tree.png}
    \caption{scenario tree for the multi-stage formulation}
    \label{fig:scenarios_tree}
\end{figure}
...
each day's operating sub-problem independent given the installed capacities.

The closure is a device of the reduced planning problem and must not be carried over to the
relaxation of Proposition~\ref{prop:bound}. That relaxation minorises the cost of a
trajectory produced by the rule-based controller, and no rule requires the stored energy to
return to its opening level at any horizon: a year that ends on a cloudy week ends with a
depleted pack. Imposing~\eqref{eq:soc-cyclic} on the minorant therefore constrains it away
from the trajectory it is meant to bound, and the bound loses its validity. This is not
hypothetical --- imposed over a week of the reference site, the closure raised the ``lower''
bound some fifteen per cent above the rule trajectory's own cost, inverting the inequality.
Over the full year the relaxation is consequently solved with a free terminal state, which
relaxes every terminal condition and so remains a valid minorant of any of them. The stored
...
\begin{equation}
  ...
  \label{eq:soc-bounds}
\end{equation}
\subsubsection{Coupling architecture}
\label{sec:coupling}

What the hybrid inverter has to carry is not a property of the plant alone but of where the
array is joined to it, and the two arrangements in use differ both in physics and in price.
Let $\alpha_n \in \{\textsc{dc}, \textsc{ac}\}$ denote that choice, decided alongside the
capacities rather than fixed beforehand, and split each source by destination:
$p^{\mathrm{pv}\to\ell}$, $p^{\mathrm{pv}\to\mathrm{b}}$ for the array,
$p^{\mathrm{ge}\to\ell}$, $p^{\mathrm{ge}\to\mathrm{b}}$ for the generator, so that
$p^{\mathrm{c}} = p^{\mathrm{pv}\to\mathrm{b}} + p^{\mathrm{ge}\to\mathrm{b}}$.

Under \textsc{dc} coupling the array feeds the battery's own bus through the trackers
integrated in the hybrid inverter, and only what leaves that bus for the load is converted.
Under \textsc{ac} coupling the array feeds the load's bus through its own string inverters
and reaches the load without conversion, while everything entering or leaving storage
crosses the hybrid inverter. Hence
\begin{equation}
  \begin{aligned}
  \alpha_n = \textsc{dc}:\quad
    & p^{\mathrm{pv}\to\ell}_{n,t,h} + p^{\mathrm{d}}_{n,t,h}
      + p^{\mathrm{ge}\to\mathrm{b}}_{n,t,h} \;\leq\; \mathrm{cap}^{\mathrm{inv}}_n, \\
  \alpha_n = \textsc{ac}:\quad
    & p^{\mathrm{c}}_{n,t,h} + p^{\mathrm{d}}_{n,t,h}
      \;\leq\; \mathrm{cap}^{\mathrm{inv}}_n,
  \end{aligned}
  \qquad \forall (n,t,h),
  \label{eq:inv-bounds}
\end{equation}
together with the battery's own admissible charge rate,
$p^{\mathrm{c}}_{n,t,h},\, p^{\mathrm{d}}_{n,t,h} \leq
 C\,\mathrm{cap}^{\mathrm{batt}}_n$.

The array is not free of the converter that admits it. A hybrid inverter's integrated
trackers accept a bounded array per unit of rating --- current three-phase units publish
thirty-nine kilowatts of array for thirty of inverter --- and an alternating-coupled array
is held at parity, since off grid whatever the load does not take must be absorbable by the
battery inverter the instant the load drops. Writing $\rho(\alpha)$ for that ratio,
\begin{equation}
  \mathrm{cap}^{\mathrm{pv}}_n \;\leq\; \rho(\alpha_n)\, \mathrm{cap}^{\mathrm{inv}}_n,
  \qquad \forall n.
  \label{eq:array-ratio}
\end{equation}
Constraint~\eqref{eq:array-ratio} is what makes enlarging the array and enlarging the
converter one decision rather than two, and it is not a refinement: on the reference
instance it removes some forty-seven per cent of the Cartesian lattice as plants that
cannot be wired. Nothing in the model sizes charge controllers outside the hybrid inverter.
Two regulators on one lithium bank must answer to the same battery-management conversation
if they are not to negotiate the charge current against each other, which confines the
arrangement to a single manufacturer's ecosystem; the conversion that \textsc{dc} coupling
needs is therefore bought with the inverter, and only the string inverters of \textsc{ac}
coupling appear as a separate cost.

Because the two architectures constrain the same variables differently, a box of the search
spans capacities but never architectures: each is certified over its own lattice and the
cheaper optimum is reported, with the margin over the other.
...
the following sunrise, clipped to the installed energy capacity.

An earlier version of this formulation proposed to impose~\eqref{eq:night-reserve} as a
hard floor and asserted that the resulting programme reproduces the field controller. It
does not, on two counts, and the distinction turns out to be the whole subject of this
work rather than a detail of encoding.

First, the floor is not satisfied by the controller it is supposed to describe. The
reserve governs whether the battery is \emph{permitted to serve the load}; it does not
prevent the battery being drawn down when the generator is saturated or absent, in which
case the controller discharges to the protective trip rather than shed load. Measured on
the reference site, the rule trajectory sits below $R_{n,t,h}$ in between four and sixty
per cent of the hours depending on how the plant is sized, by as much as seventy-five
kilowatt-hours. Imposed as a constraint, \eqref{eq:night-reserve} would therefore exclude
the very behaviour it claims to encode, and a bound computed under it could exceed the
trajectory it is meant to minorise --- the same failure as the cyclic closure of
Section~\ref{sec:coupling}, and for the same reason.

Second, and more fundamentally, a programme is not a controller. Adding the floor to the
cost objective yields an \emph{anticipative} optimisation: it minimises over the whole
horizon knowing the whole horizon. The field controller is \emph{causal}, deciding hour by
hour with a bounded look-ahead and no knowledge of the weather beyond it. The distance
between the two is exactly $\Delta_{\mathrm{heur}}$, the quantity this work sets out to
measure, so encoding the rule as a programme would measure the cost of the reserve
constraint in place of the cost of myopia.

The operating layer is therefore represented twice, and the two representations are of
different kinds. The \emph{lower} oracle is the programme
\eqref{eq:balance}--\eqref{eq:inv-bounds} under the cost objective alone, whose optimum
$C^{\mathrm{opt}}_n(x)$ is computable and minorises the controller by
Proposition~\ref{prop:bound}. The \emph{upper} oracle is the controller itself, executed
step by step as the algorithm it is; no formulation of it is used, and none is needed.
Constraint~\eqref{eq:night-reserve} is retained in the notation because the reserve is a
quantity the controller computes, not because it is imposed on any programme.

A softened form of the floor --- required only in the hours when the generator is not
already at its rating, which is precisely when the controller does respect it --- would
give a valid and strictly tighter lower bound than the cost objective alone. That is
recorded here as an opening rather than a result.
...
\begin{description}[leftmargin=2.4cm,style=nextline]
  ...
  socio-economic pathways SSP1-2.6, SSP2-4.5 and SSP3-7.0, each of which yields
  a distinct trajectory of irradiance and ambient temperature and hence of the specific
  yield $Y$. The high-forcing pathway SSP5-8.5 is deliberately excluded: its emission
  trajectory is now widely regarded as implausible rather than as a business-as-usual
  baseline, and retaining it would place probability mass on a future that inflates the
  spread of the resource scenarios without informing the sizing decision.
